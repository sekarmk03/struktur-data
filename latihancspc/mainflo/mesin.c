...
void addFirst(float flo, list *L) {
...
void addAfter(element* prev, float flo, list *L) {
    element *new;
...
void addArange(float flo, list *L) {
    if((*L).first == NULL) {
        ...
        if((flo - (int)flo) < ((*L).first->container.flo - (int)(*L).first->container.flo)) {
            addFirst(flo, L);
        } else {
            ...
            if((flo - (int)flo) > (last->container.flo - (int)last->container.flo)) {
                addLast(flo, L);
            } else {
                element *bantu = (*L).first;
                int stop = 0;
                while(bantu->next != NULL && stop != 1) {
                    if((flo - (int)flo) < (bantu->container.flo - (int)bantu->container.flo)) {
                        stop = 1;
                    } else {
                        prev = bantu;
                        bantu = bantu->next;
