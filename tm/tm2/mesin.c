...
void addFirst(char name[], char formula[], char concent[], list *L) {
    ...
    strcpy(new->container.formula, formula);
    strcpy(new->container.concent, concent);
...
void addAfter(element* prev, char name[], char formula[], char concent[], list *L) {
    element *new;
...
void addLast(char name[], char formula[], char concent[], list *L) {
    if((*L).first == NULL) {
        // jika list kosong jalankan prosedur addFirst()
        addFirst(name, formula, concent, L);
    } else {
        ...
        addAfter(prev, name, formula, concent, L);
    }
}
...
void printElement(list L) {
    if(L.first != NULL) {
        ...
        while(flag != NULL) {
            ...
            printf("%s %s %s\n", flag->container.name, flag->container.formula, flag->container.concent);
            flag = flag->next;
            i++;
        }
    } else {
